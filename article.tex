...
&\gap\gap \Gamma \ts S \wf \andl t_i \in \rele j {\Gamma \envplus{x : S}} s U)\\
&\gap \andl\\
...
a method invocation on uninitialized method.

Note that this relation doesn't directly impose constraints on the
type labels. In particular, since we're just enforcing appropriate
initializations of all fields and methods by looking at an expansion,
lower bounds of type selections are completely ignored. This is by
design, because a term doesn't get stuck in a method invocation if
its argument type doesn't satisfy the lower bound required by a type
selection -- the upper bound is what matters operationally.

However, a {\it potential pitfall} is that the value itself might
occur in the types $S$, $U$, $V$, because we substitute it for the
``self'' occurrences in the expansion.
...
k$. The non-trivial case is when $k > 0$ and for some $j < k$,
$\reductionl {t} s {t'} {s'} j \andl \irred {t'} {s'}$. Then, we need
to show ${t'} \in \relv {k-j-1} {\Gamma''} {s'} T$ for ${\Gamma''} \in
\relg \Gamma k {s'} {\Gamma'}$.
...
\Gamma s U$, we have fewer definitions to check than for $v \in \relv
k \Gamma s S$.

There are two {\it potential pitfalls}.

First, because we don't check for type labels, we can end up in a
situation where a type label defined in both $S$ and $U$ is actually
only used meaningfully in $U$, and so $v \in \relv k \Gamma s S$ never
enforces it. Consider $v$ to map to $\Top \refine z {\ldecl l \Top}
\ldefs{l = v}$, $S$ is $\Top \refine z {\Ldecl L \Top \Top , \ldecl l
  \Top}$ and $U$ is $\Top \refine z {\Ldecl L \Top \Top , \ldecl l
  {z.\Top}}$. Now, $v \in \relv k \Gamma s S$ even though it doesn't
define the type label $L$. But $v \not\in \relv k \Gamma s U$ because
$v.L$ is not well-formed. \remark{Counterexample!}

Another worry is potential narrowing issues for methods: because the
parameter type is contravariant, $U$ can require a more precise type
than $S$. This should be fine, because if the parameter type becomes
non-expanding, then the check is vacuously true.
\end{proofsketch}
