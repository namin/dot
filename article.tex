...
\fun x S T t &\iff \new z {\arrow S T} \ldefs{\mi{apply}(x) = t}\\
...
We prove that the DOT calculus is type-safe using logical relations in
section~\ref{dot-type-safety}. However, we first tried to prove the
calculus type-safe using the standard theorems of preservation and
progress. Unfortunately, for the calculus as presented, and any of its
variants that we devised, preservation doesn't hold. In this section,
we review some of the most salient counterexamples to preservation
that we found. These counterexamples have been checked with PLT Redex.
...
the need for acknowledging path equality for preservation to hold.
...
Originally, subtyping was not regular with respect to $\wfe$. Roughly,
...
function type to have a special marker value label.
...
example doesn't typecheck initially because the path-equality
provisions only applies when objects are in the store, so the
application is not well-typed. But if we start preservation in a store
which has $a$, $b$, $c$ and $d$ then the application type-checks,
because, through one of the path-equality provision, we can find that
the type of $d$ is a subtype of $\Bot$. Now, of course, when we get to
$d.\mathit{foo}$, reduction fails.
