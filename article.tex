...
&\gap\gap \Gamma \ts v.L_i \wfe \spcomma S \wf \spcomma U \wf \spcomma S \sub U\\
&\gap (\forall m_i : S \rightarrow U \in \seq{D},\\
&\gap\gap \Gamma \ts S \wf \spcomma U \wf \andl t_i \in \rele j {\Gamma \envplus{x : S}} s U)\\
&\gap \andl\\
...
expansion, so the relation makes sure that the type labels exist.
