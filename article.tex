...
&\gap\gap t_i \in \rele k {\Gamma \envplus{x : S}} s U)\\
&\gap \andl\\
&\gap (\forall l_i : V \in \seq{D},\\
&\gap\gap v_i \in \relv k \Gamma s V)\\
&\}
\end{align*}
...
&\gap\gap \forall s' \in \relg k \Gamma s ,\\
&\gap\gap \reduction t {s'} {t'} {s''} \andl\\
...
\begin{align*}
&\relg k \Gamma s = \{ s, \seq{x_i \mapsto c_i}^{m \leq i < n} \ |\\
