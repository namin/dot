...
T_1$, $\Gamma \ts T_1 \expand_z \seq{D}$, $l_i : V_i \in \seq{D}$ and
knowing either that $t_1 = p_1 \andl T = \subst p z {V_i}$ or that $z
\not\in \fn(V_i) \andl T = V_i$.

Need to show $t_1.l_i \in \rele k \Gamma - T$. Suppose $k > 0$ and $s
\in \rels k \Gamma -$. Suppose $\reductionl {t_1.l_i} s {t'} {s'}
\andl \irred {t'} {s'} \andl \Gamma' \in \relg \Gamma k {s'}
\Gamma$. Remains to show ${t'} \in \relv {k-j-1} {\Gamma'} {s'} T$.

By operational semantics, $\reductionl {t_1} s {t_1'} {s'} {j-1}$ and
$\irred {t_1'} {s'}$. By induction hypothesis, ${t_1'} \in \relv
{k-j+1-1} {\Gamma'} {s'} {T_1}$.
