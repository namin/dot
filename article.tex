...
$\Gamma \ds t : T$ is simply defined as $t \in \rele k \Gamma - T,
\forall k$.
...
each case, we need to show $t \in \rele k \Gamma - T, \forall k$. The
non-trivial case is to need to show ${t'} \in \relv {k-j-1} {\Gamma'}
{s'} T$ by supposing $k > 0$, $s \in \rels k \Gamma -$, and, for some
...
%Need to show $\new y c {t'} \in \rele k \Gamma - T$. Suppose $k > 0$
%and $s \in \rels k \Gamma -$. Suppose, for some $j \leq k$,
