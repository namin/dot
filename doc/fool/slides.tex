...
The term type-checks because the term $t=\app {(\fun y {\arrow \Top Y} Y
  {\app y u})} {(\fun d \Top Y {\cast X u})}$ has type
...
\item See the paper for the entire formalism, examples,
  counterexamples, and discussion of type-safety, design decisions and
  variants.
\end{itemize}
\end{frame}
