...
and structural typing through the use of refinement types.

DOT does not precisely describe what's currently in Scala: it is more
normative than descriptive, since our goal is to explore
simplifications and unifications of concepts in the type system. In
particular, DOT unifies Scala's constructs for type members, and
provides classical intersection and union types, which simplify
Scala's compound types and glb/lub computations.
...
mixture of nominal and structural typing. Compared to previous
approaches~\cite{nuObj,FS}, we make no attempt to model inheritance or
mixin composition. Indeed we will argue that such concepts are better
modeled in a different setting.
...
member is declared by its lower and upper bounds. Furthermore, unlike
Scala, there is no notion of uninstiated type member. Here is an
example inspired by~\cite{vt}:
\begin{lstlisting}
trait Food
trait Animal {
  type Meal <: Food
...
\end{lstlisting}
In Scala, \code{Meal} is an uninstiated type member of \code{Animal}:
it is not possible to instantiate an \code{Animal} without providing a
concrete binding for \code{Meal}. In DOT, \code{Meal} is a type member
with a lower bound of $\Bot$ and an upper bound of \code{Food}. It is
possible to instantiate an \code{Animal} without further specifying
\code{Meal}, though it would be not possible to feed it. Now,
we define \code{Cow} by refining \code{Animal}.
\begin{lstlisting}
...
binding for \code{Meal}. Now, we can instantiate \code{Cow}s and feed
them \code{Grass}. In DOT, such a type alias is declared by giving it
identical lower and upper bounds.

DOT has a notion of concrete vs abstract type members, but it used to
distinguish which types members are instantiable as opposed to
instantiated. In the example above, all types introduced by
\code{trait} would be concrete type members in DOT with lower bounds
of $\Bot$ and upper bounds describing the \code{extends} clause:
$\Top$ for \code{Food}, a refinement of $\Top$ with type member
\code{Meal} and method member \code{eat} for \code{Animal},
\code{Food} for \code{Grass} and a refinement of \code{Animal} with
type member \code{Meal} for \code{Cow}. Concrete type members
typically have a lower bound of $\Bot$ so that they are purely
nominal: e.g. one needs to directly or indirectly instantiate a
...
\subsection{Scala to DOT}\label{dot-examples-scala-sugar}
// TODO
...
list package to manipulate lists with a particular element type.
\begin{align*}
...
&\ \gap \mdecl {\mi{head}} {\mi{root}.\mi{Unit}} {p.\mi{Elem}}\\\displaybreak[3]
&\ \gap \mdecl {\mi{tail}} {\mi{root}.\mi{Unit}} {p.\mi{List}}\\\displaybreak[3]
&\gap}}\\\displaybreak[3]
&\ \mdecl {\mi{nil}} {\mi{root}.\mi{Unit}} {p.\mi{List}}\\\displaybreak[3]
&\ \mdecl {\mi{cons}} {p.\mi{Elem}} {\arrow {p.\mi{List}} {p.\mi{List}}}\\\displaybreak[3]
&}}}}{
...
&\ {\mi{cons}}(hd) = \ldots\\\displaybreak[3]
&}}}{
\ldots
}
\end{align*}
