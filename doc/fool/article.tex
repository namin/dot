...
Scala, there is no notion of uninstantiated type member. Here is an
...
\end{lstlisting}
In Scala, \code{Meal} is an uninstantiated type member of \code{Animal}:
...
Using lower bounds of $\Bot$ for the abstract type members ensures
they can vary covariantly. On the other hand, $\mi{choose}$ cannot be
called meaningfully, unless $A$ and $B$ have realizable lower bounds.
...
$a$ enforces that it's first argument be a $\mi{pets.Poodle_c}$ and it's second a $\mi{pets.Dalmatian_c}$. The following does not typecheck for this reason:
\begin{align*}
