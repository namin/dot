...
In this paper, we present and motivate the DOT calculus, our core
calculus for path-dependent types, both formally and informally. We
also discuss our work-in-progress to prove type-safety of the calculus.
\end{abstract}
...
Scala's type inference. For example, in to infer the type \code{if} expression such as
\begin{lstlisting}
...
We propose DOT as a core calculus for path-dependent types. We first
motivate the calculus informally in section~\ref{dot-motiv}. We
present the calculus formally in section \ref{dot-calculus} and
through examples in section~\ref{dot-examples}. Though we show that
the calculus does not satisfy the standard theorem of preservation in
...
\section{Motivation}\label{dot-motiv}
%TODO
