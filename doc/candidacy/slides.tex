...
\begin{itemize}
\item variables $x$ denoting objects
\item selections $t.l$
\item object creations
\item \mbox{$\newin{x \larrowc t}{u}$} defining an object $x$ of class $t$
whose scope is the term $u$
\item class templates $\cls{x: S \sep \tup d}$,
\item mixin compositions $t \withsub{S} u$ denoting a class of self
type $S$ combined from two classes $t$ and $u$
\end{itemize}
