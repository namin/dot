...
Our aim is to propose a new type-theoretic foundation of Scala based
on a core calculus for path-dependent types. Towards this end, we
review three papers presenting calculi in which path-dependent types
are essential: a calculus for first-class translucent ML-like modules,
the Tribe calculus for virtual classes, and the ${\nu}Obj$ calculus
for reasoning about classes and objects with type members.
...
\code{type T} is an abstract type member of \code{AbsCell}. In a
library method taking an abstract cell \code{c}, the expression
\code{c.set(c.init)} is well-typed because \code{c.init} has type
\code{c.T} and \code{c.set} has the type of a function that expects a
parameter of type \code{c.T}. Note that \code{c.T} is a path-dependent
type. \lstinputlisting{intro.scala}
...
\section{Survey of the Selected Papers}\label{survey}%: The Giants upon whose shoulders I will stand
...
from an \code{if} expression. In fact, allowing this might require obscuring
...
of the paper. Since modules are first-class, we can \code{flip} a coin
...
\code{X}? In ML, in order to make the \code{if} expression typecheck, the two
...
inner classes are inherited along with their methods and fields. {\it
...
coming from different families. Yet, thanks to {\it family
  polymorphism}, code written for one family also works for extensions
of that family. In Tribe, path types can depend simultaneously on both
classes and objects. Additionally, paths can use an \code{out} field
to move from an object to the object which surrounds it. This
\code{out} feature enables ubiquitous access to an object's family
without the need to drag around family arguments.
...
In Tribe, types are a form of generalized paths: a final variable,
\code{this} or a class name then followed by a possibly empty sequence
of field, \code{out} or class selections. Given the code below, we can read the
type \code{kitt.Passenger.name} as the name of one of Kitt's
passengers (including the driver). Subtyping is natural:
\code{kitt.driver <: Car.driver <: Car.Passenger <: Car.Traveller <: Vehicle.Traveller}. Crucially, \code{kitt.driver} is not a subtype
of \code{karr.driver} and similarly for \code{kitt.Passenger} and
\code{karr.Passenger}.
\begin{lstlisting}
...
substitution lemma does not seem valid when \code{null} is the value to be
...
syntactic overhead, since further binding and \code{out} selection are
...
leaf classes \code{Graph} and \code{ColouredGraph} are no longer in a
...
\code{out} faithfully involves a bit of hackery in making Scala recognize
that, for example, \code{e.from.type} is a subtype of
\code{e.out.Node}. As noted in the Tribe paper, using \code{out} in library
code obscures the symmetry between the parameters (for example, in the
\code{distance} method below) but allows the family object to be
recovered without being explicitly passed around (contrast with the
method \code{distance2}).
\lstinputlisting{tribe.scala}
...
\section{Research Proposal}\label{proposal}%: How I Plan to Revolutionize the World
