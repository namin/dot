...
synctactic overhead, since further binding and ``out'' selection are
not native. Emulating ``out'' faithfully involves a bit of hackery in
making Scala recognize that, for example, \code{e.from.type} is a
subtype of \code{e.out.Node}. As noted in the paper, using ``out'' in
library code obscures the symmetry between the parameters (for
example, in the \code{distance} method below) but allows the family
object to be recovered without being explicitly passed around
(contrast with the method \code{distance2}).
