...
\item The second paper, {\it Tribe: a simple virtual class calculus}
  ...
  path.
\item The third paper, {\it A Nomial Theory of Objects with Dependent
  ...
  any path $p$ has a singleton type $p.\type$.
\end{itemize}
...
module system is that ``type sharing'' is limited to equality between
...
\code{X}? In ML, in order to make the \code{if} type check, the two
...
syntactic overhead, since further binding and ``out'' selection are
...
subclassing relationship with one another, as this would only bring
confusion when ascribing a coloured graph to a graph. Emulating
``out'' faithfully involves a bit of hackery in making Scala recognize
that, for example, \code{e.from.type} is a subtype of
\code{e.out.Node}. As noted in the paper, using ``out'' in library
...
devise, preservation doesn't hold. Indeed, we found many
...
are exploring a promising avenue to prove type safey using the
