...
{\reduction {\mapp o {m_i} v} s {\subst v {x_i} {t_i}} s}
...
\Gamma \envplus{y: T_c} \ts \seq{S <: U} \spcomma \seq{d \typ D} \spcomma {t'} \typ {T} \spcomma T <: T'}
...
\infrule[\textsc{vdecl}]
{\Gamma \ts w \typ {V'} \spcomma {V'} \sub V}
{\Gamma \ts (l = w) \typ (\ldecl l V)}
...
\caption{The DOT Calculus : Type / Declaration Assignment}\label{dot-one-bis}
\end{figure}
...
\begin{figure}
 \figurebox{
    ...
    \begin{multicols}{2}[\judgement{Membership}{\fbox{$\Gamma \ts t \ni D$}}]
      ...
      \infrule[\textsc{term-$\ni$}]
      {\forall p, t \neq p \andalso z \not\in \fn(D_i)\\
       \Gamma \ts t \typ T \spcomma T <: T' \spcomma T' \expand_z \seq D}
      {\Gamma \ts t \ni D_i}
   \end{multicols}
