...
\pdfbookmark[0]{Expansion}{expansion}
    \begin{multicols}{2}[\judgement{Expansion}{\fbox{$\Gamma;s \ts T \anyexpand_z \seq D$}}]
      ...
      {\Gamma;s \ts \Top \anyexpand_z \{\}}

      \infrule[\textsc{$\tand$-$\expand$}]
      {\Gamma;s \ts T_1 \anyexpand_z {\seq D_1} \spcomma M/T_2 \anyexpand_z {\seq D_2}}
      {\Gamma;s \ts T_1 \tand T_2 \anyexpand_z {\seq D_1 \tand \seq D_2}}

      \infrule[\textsc{rfn-$\expand$}]
      {\Gamma;s \ts T \anyexpand_z {\seq {D}}}
      {\Gamma;s \ts T \refine z {\seq D} \anyexpand_z \seq {D} \tand \seq D}

      \infax[\textsc{$\Bot$-$\expand$}]
      {\Gamma;s \ts \Bot \anyexpand_z \seq{D_\Bot}}

      \infax[\textsc{any-$\weakexpand$}]
      {\Gamma;s \ts M/T \weakexpand_z \{\}}

      \infrule[\textsc{$\tor$-$\weakexpand$}]
      {\Gamma;s \ts T_1 \weakexpand_z {\seq D_1} \spcomma M/T_2 \weakexpand_z {\seq D_2}}
      {\Gamma;s \ts T_1 \tor T_2 \weakexpand_z {\seq D_1 \tor \seq D_2}}

      \infrule[\textsc{tsel-$\expand_2$}]
      {{s \ts p.L \optresolves q.L}\\
       {\Gamma;s \ts q \ni \Ldecl L S U}\\
       {\Gamma;s \ts U \anyexpand_z \seq D}}
      {\Gamma;s \ts p.L \anyexpand_z \seq D}
    \end{multicols}
}
