...
\infrule[\textsc{new}]
{
\Gamma;s \ts T_c \wf \spcomma T_c \expand_y \seq{\Ldecl L S U}, \seq{D} \\
...
\pdfbookmark[0]{Membership}{membership}
      ...
      \infrule[\textsc{path-$\ni$}]
      {\Gamma;s \ts p \typ T \spcomma T \weakexpand_z \seq D}
      {\Gamma;s \ts p \ni \subst p z {D_i}}

   \linesep
\pdfbookmark[0]{Expansion}{expansion}
    \begin{multicols}{2}[\judgement{Expansion}{\fbox{$\Gamma;s \ts T \anyexpand_z \seq D$}}]
      \infrule[\textsc{any-$\anyexpand$}]
      {\Gamma;s \ts \emptyset / T \anyexpand'_z \seq D}
      {\Gamma;s \ts T \anyexpand_z \seq D}
    \end{multicols}

    \begin{multicols}{2}[\judgement{Expansion'}{\fbox{$\Gamma;s \ts M / T \anyexpand'_z \seq D$}}]
      \infax[\textsc{$\Top$-$\expand'$}]
      {\Gamma;s \ts M/\Top \anyexpand'_z \{\}}

      \infrule[\textsc{$\tand$-$\expand'$}]
      {\Gamma;s \ts M/T_1 \anyexpand'_z {\seq D_1} \spcomma M/T_2 \anyexpand'_z {\seq D_2}}
      {\Gamma;s \ts M/T_1 \tand T_2 \anyexpand'_z {\seq D_1 \tand \seq D_2}}

      \infrule[\textsc{rfn-$\expand'$}]
      {\Gamma;s \ts M/T \anyexpand'_z {\seq {D'}}}
      {\Gamma;s \ts M/T \refine z {\seq D} \anyexpand'_z \seq {D'} \tand \seq D}

      \infrule[\textsc{tsel-$\expand'_1$}]
      {{s \ts p.L \optresolves q.L}\\
       {q.L \in M}}
      {\Gamma;s \ts M / p.L \weakexpand'_z \{\}}

      \infax[\textsc{$\Bot$-$\expand'$}]
      {\Gamma;s \ts M/\Bot \anyexpand'_z \seq{D_\Bot}}

      \infrule[\textsc{$\tor$-$\expand'$}]
      {\Gamma;s \ts M/T_1 \weakexpand'_z {\seq D_1} \spcomma M/T_2 \weakexpand'_z {\seq D_2}}
      {\Gamma;s \ts M/T_1 \tor T_2 \weakexpand'_z {\seq D_1 \tor \seq D_2}}

      \infrule[\textsc{tsel-$\expand'_2$}]
      {{s \ts p.L \optresolves q.L}\\
       {q.L \not\in M}\\
       {\Gamma;s \ts q \ni \Ldecl L S U}\\
       {\Gamma;s \ts M,q.L / U \anyexpand'_z \seq D}}
      {\Gamma;s \ts M / p.L \anyexpand'_z \seq D}

      \infax[\textsc{any-$\weakexpand'$}]
      {\Gamma;s \ts M/T \weakexpand'_z \{\}}
    \end{multicols}
}
...
%      {\Gamma \ts S \sub U}
      ...
      {\Gamma;s \ts p_1.L \wf \spcomma p_2.L \wf\\
      ...
      {\Gamma;s \ts {T \refine z {\seq D}} \wf
       \spcomma S \sub T \spcomma S \weakexpand_z \seq{D'} \\
      ...
      {\Gamma;s \ts T_2 \wf \spcomma T \sub T_1}
      {\Gamma;s \ts T \sub T_1 \tor T_2}

      \infrule[\textsc{$\sub$-$\tor_2$}]
      {\Gamma;s \ts T_1 \wf \spcomma T \sub T_2}
      {\Gamma;s \ts T \sub T_1 \tor T_2}
      ...
      {\Gamma;s \ts T_2 \wf \spcomma T_1 \sub T}
      {\Gamma;s \ts T_1 \tand T_2 \sub T}

      \infrule[\textsc{$\tand_2$-$\sub$}]
      {\Gamma;s \ts T_1 \wf \spcomma T_2 \sub T}
      {\Gamma;s \ts T_1 \tand T_2 \sub T}
