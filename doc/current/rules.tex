...
\begin{figure}
\figurebox{
\begin{multicols}{2}[\judgement{Type Assignment}{\fbox{$\Gamma;s \ts t \typ T$}}]
...
\infrule[\textsc{msel}]
{\Gamma;s \ts t \ni m \typ {S \tfun T} \\
 \Gamma;s \ts t' \typ T' \spcomma T' \sub S}
{\Gamma;s \ts \mapp t m {t'} \typ T}
...
\Gamma \envplus{y: T_c};s \ts \seq{S <: U} \spcomma \seq{d \typ D} \spcomma {t'} \typ {T} \spcomma T <: T'}
...
\begin{multicols}{2}[\judgement{Declaration Assignment}{\fbox{$\Gamma;s \ts d \typ D$}}]

\infrule[\textsc{vdecl}]
{\Gamma;s \ts w \typ {V'} \spcomma {V'} \sub V}
{\Gamma;s \ts (l = w) \typ (\ldecl l V)}

\infrule[\textsc{mdecl}]
{\Gamma;s \ts S \wfe\\
 \Gamma;s \envplus{x : S} \ts t \typ {T'} \spcomma {T'} \sub T}
{\Gamma;s \ts (m(x) = t) \typ (\mdecl m S T) }
...
\begin{figure}
 \figurebox{
    ...
    \begin{multicols}{2}[\judgement{Membership}{\fbox{$\Gamma;s \ts t \ni D$}}]
      ...
      \infrule[\textsc{term-$\ni$}]
      {\forall p, t \neq p \andalso z \not\in \fn(D_i)\\
       \Gamma;s \ts t \typ T \spcomma T <: T' \spcomma T' \expand_z \seq D}
      {\Gamma;s \ts t \ni D_i}
   \end{multicols}
...
\begin{figure}
  \figurebox{
    ...
    \begin{multicols}{2}[\judgement{Path Reduction}{\fbox{$s \ts p_1 \reduces p_2$}}]
