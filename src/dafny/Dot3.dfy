  ...
  decreases n;
{
  ...
  case tm_new(y, Tc, init, t') =>
    ...
    wf_init(n-1, Context(Extend(y, Tc, ctx.m)), s, Ds.decls, def_seq_sort(lst2seq(init))) &&
    !tp_fn(y, T) &&
    typing(n-1, Context(Extend(y, Tc, ctx.m)), s, t', T)
  case tm_sel(t, l) =>
  ...
  decreases n;
{
  n>0 && (
  (decls==[] && defs==[]) || (|decls|>0 && (
    (((decls[0].decl_tp_c? && subtype(n-1, ctx, s, decls[0].Sc, decls[0].Uc)) ||
      (decls[0].decl_tp_a? && subtype(n-1, ctx, s, decls[0].Sa, decls[0].Ua))) &&
    ...
    ((decls[0].decl_tm? && defs[0].def_tm? && decls[0].l==defs[0].l &&
     ...
     (decls[0].decl_mt? && defs[0].def_mt? && decls[0].m==defs[0].m &&
      ...
      exists T' :: typing(n-1, Context(Extend(defs[0].param, decls[0].P, ctx.m)), s, defs[0].body, T') &&
      subtype(n-1, Context(Extend(defs[0].param, decls[0].P, ctx.m)), s, T', decls[0].R))) &&
...
{
  match d
  case decl_tp_c(L, S, U) => n>0 && wf_type(n-1, ctx, s, S) && wf_type(n-1, ctx, s, U)
  case decl_tp_a(L, S, U) => n>0 && wf_type(n-1, ctx, s, S) && wf_type(n-1, ctx, s, U)
  case decl_tm(l, T) => n>0 && wf_type(n-1, ctx, s, T)
  case decl_mt(m, S, T) => n>0 && wf_type(n-1, ctx, s, S) && wf_type(n-1, ctx, s, T)
}
predicate wf_decls(n: nat, ctx: context, s: store, Ds: list<decl>)
  decreases n;
{
  match Ds
  case Nil => true
  case Cons(head, tail) => n>0 && wf_decl(n-1, ctx, s, head) && wf_decls(n-1, ctx, s, tail)
}
  ...
  decreases n;
{
  match T
  case tp_rfn(T', z, Ds) =>
    n>0 && wf_type(n-1, ctx, s, T') && wf_decls(n-1, Context(Extend(z, T, ctx.m)), s, Ds)
  case tp_sel_c(p, L) =>
    path(p) &&
    n>0 && exists S, U :: concrete_type_membership(n-1, ctx, s, p, L, S, U)
  case tp_sel_a(p, L) =>
    path(p) &&
    n>0 && exists S, U :: abstract_type_membership(n-1, ctx, s, p, L, S, U)
...
predicate concrete_type_membership(n: nat, ctx: context, s: store, t: tm, L: nat, S: tp, U: tp)
  decreases n;
{
  n>0 && exists d :: membership(n-1, ctx, s, t, L, d) &&
  d.decl_tp_c? && d.Lc==L && d.Sc==S && d.Uc==U
}
predicate abstract_type_membership(n: nat, ctx: context, s: store, t: tm, L: nat, S: tp, U: tp)
  decreases n;
{
  n>0 && exists d :: membership(n-1, ctx, s, t, L, d) &&
...
predicate expansion(n: nat, ctx: context, s: store, z: nat, T: tp, Ds: decls)
  ensures expansion(n, ctx, s, z, T, Ds) && Ds.decls_fin? ==> decl_seq_sorted(Ds.decls);
  decreases n;
{
  n>0 && expansion_iter(n-1, Empty, ctx, s, z, T, Ds)
}
predicate expansion_iter(n: nat, m: partial_map<tp, decls>, ctx: context, s: store, z: nat, T: tp, Ds: decls)
  requires m_decl_seq_sorted(m);
  ensures expansion_iter(n, m, ctx, s, z, T, Ds) && Ds.decls_fin? ==> decl_seq_sorted(Ds.decls);
  decreases n;
{
  match T
  case tp_rfn(T', z', Ds') =>
  ...
  case tp_sel_c(p, L) =>
    ...
    (n>0 && lookup(T, m).None? && exists S, U :: concrete_type_membership(n-1, ctx, s, p, L, S, U) && expansion_fix(n-1, T, decls_fin([]), m, ctx, s, z, U, Ds))
  case tp_sel_a(p, L) =>
    (lookup(T, m).Some? && lookup(T, m).get==Ds && (Ds.decls_fin? ==> decl_seq_sorted(Ds.decls))) || 
    (n>0 && lookup(T, m).None? && exists S, U :: abstract_type_membership(n-1, ctx, s, p, L, S, U) && expansion_fix(n-1, T, decls_fin([]), m, ctx, s, z, U, Ds))
  case tp_and(T1, T2) =>
    n>0 && exists Ds1, Ds2 :: expansion_iter(n-1, m, ctx, s, z, T1, Ds1) && expansion_iter(n-1, m, ctx, s, z, T2, Ds2) &&
...
predicate expansion_fix(n: nat, selT: tp, selDs: decls, m: partial_map<tp, decls>, ctx: context, s: store, z: nat, T: tp, Ds: decls)
  requires m_decl_seq_sorted(m);
  requires selDs.decls_fin? ==> decl_seq_sorted(selDs.decls);
  ensures expansion_fix(n, selT, selDs, m, ctx, s, z, T, Ds) && Ds.decls_fin? ==> decl_seq_sorted(Ds.decls);
  decreases n;
{
  n>0 && (
  (selDs==Ds && expansion_iter(n-1, Extend(selT, selDs, m), ctx, s, z, T, Ds)) ||
  (selDs!=Ds && exists Da :: expansion_iter(n-1, Extend(selT, selDs, m), ctx, s, z, T, Da) &&
...
{
  match d1
  case decl_tp_c(L, S, U) => n>0 && subtype(n-1, ctx, s, d2.Sc, S) && subtype(n-1, ctx, s, U, d2.Uc)
  case decl_tp_a(L, S, U) => n>0 && subtype(n-1, ctx, s, d2.Sa, S) && subtype(n-1, ctx, s, U, d2.Ua)
  ...
  decreases n;
{
  ...
  case decls_fin(s1) =>
    (match Ds2
     ...
     case decls_fin(s2) => n>0 && decls_fin_sub(n-1, ctx, s, s1, s2))
}
  ...
  decreases n;
{
  ...
  /* <:-rfn */  (T.tp_rfn? && wfe_type(n-1, ctx, s, T) && subtype(n-1, ctx, s, S, T.base_tp) &&
                 ...
                 decl_seq_sorted(rfn_decls) &&
                 decls_sub(n-1, Context(Extend(T.self, S, ctx.m)), s, decls_fin(rfn_decls), Ds')) ||
  /* rfn-<: */  (S.tp_rfn? && wfe_type(n-1, ctx, s, S) && subtype(n-1, ctx, s, S.base_tp, T)) ||
  /* <:-selc */ (T.tp_sel_c? &&
                 exists S', U' :: concrete_type_membership(n-1, ctx, s, T.pc, T.Lc, S', U') &&
                 subtype(n-1, ctx, s, S', U') && subtype(n-1, ctx, s, S, S')) ||
  /* selc-<: */ (S.tp_sel_c? &&
                 exists S', U' :: concrete_type_membership(n-1, ctx, s, S.pc, S.Lc, S', U') &&
                 subtype(n-1, ctx, s, S', U') && subtype(n-1, ctx, s, U', T)) ||
  /* <:-sela */ (T.tp_sel_a? &&
                 exists S', U' :: abstract_type_membership(n-1, ctx, s, T.pa, T.La, S', U') &&
                 subtype(n-1, ctx, s, S', U') && subtype(n-1, ctx, s, S, S')) ||
  /* sela-<: */ (S.tp_sel_a? &&
                 exists S', U' :: abstract_type_membership(n-1, ctx, s, S.pa, S.La, S', U') &&
                 subtype(n-1, ctx, s, S', U') && subtype(n-1, ctx, s, U', T)) ||
  ...
  /* pathredc */(T.tp_sel_c? && wfe_type(n-1, ctx, s, T) && exists p :: path_red(ctx, s, T.pc, p) &&
                 subtype(n-1, ctx, s, S, tp_sel_c(p, T.Lc))) ||
  /* pathreda */(T.tp_sel_a? &&  wfe_type(n-1, ctx, s, T) && exists p :: path_red(ctx, s, T.pa, p) &&
                 subtype(n-1, ctx, s, S, tp_sel_a(p, T.La))))
}
  ...
  ensures field_membership'(ctx', s', t', l, Tl') && subtype'(ctx', s', Tl', Tl);
{
  ...
  //Tl' := d'.T;
  Tl' := *;
  assume field_membership'(ctx', s', t', l, Tl') && subtype'(ctx', s', Tl', Tl);
}
...
{
  assume membership'(ctx, s, t, l, decl_tm(l, Tl)); // TODO: should follow from field_membership...
...
ghost method theorem_fundamental_R_var(T: tp, x: nat, ctx: context, s: store)
  requires typing'(ctx, Store(Empty), tm_var(x), T);
  requires Xstore(ctx, s);
  ...
  decreases t;
{
  ...
  ensures E(T, t, ctx, s);
  {
    ...
    case tm_sel(t1, l) =>
      // TODO: should be deriveable from IH...
      assume field_membership'(ctx, Store(Empty), t1, l, T);
      assume membership'(ctx, Store(Empty), t1, l, decl_tm(l, T));
      assume exists T1 :: typing'(ctx, Store(Empty), t1, T1);
