...
function store_tp_lookup(n: nat, s: store): tp
  requires n in dom(s.m);
{
  lookup(n, s.m).get.fst
}
...
// Logical Relations
// -----------------
function pathev(p: tm, s: store): option<int>
  requires path(p);
  ensures pathev(p, s).Some? ==> pathev(p, s).get >= 0;
  ensures pathev(p, s).Some? ==> pathev(p, s).get in dom(s.m);
{
  match p
  case tm_var(x) =>
    if (x in dom(s.m))
  ...
  case tm_sel(p1, l) =>
    if (pathev(p1, s).Some? &&
        pathev(p1, s).get in dom(s.m) &&
        def_field_lookup(l, store_lookup(pathev(p1, s).get, s)).Some? &&
        def_field_lookup(l, store_lookup(pathev(p1, s).get, s)).get.tm_var? &&
        def_field_lookup(l, store_lookup(pathev(p1, s).get, s)).get.x in dom(s.m))
    then Some(def_field_lookup(l, store_lookup(pathev(p1, s).get, s)).get.x)
    else None
}
...
predicate store_ok(k: nat, s: store)
  decreases k, k;
{
  forall x:nat :: x in dom(s.m) ==>
    concrete(store_tp_lookup(x, s)) && (k==0 || V(k-1, x, store_tp_lookup(x, s), s))
}

predicate V(k: nat, x: nat, T: tp, s: store)
  ensures V(k, x, T, s) ==> x in dom(s.m);
  decreases k, T;
{
  match T
  case tp_bot => false
  case tp_top => x in dom(s.m)
  case tp_and(tp1, tp2) => V(k, x, tp1, s) && V(k, x, tp2, s)
  case tp_or(tp1, tp2) => V(k, x, tp1, s) || V(k, x, tp2, s)
  case tp_rfn(tp1, z, ds) => V(k, x, tp1, s) && (k==0 ||
    (forall Li:nat, Si, Ui ::
      (decl_tp_c(Li, Si, Ui) in lst2seq(decls_subst(z, tm_var(x), ds)) ||
       decl_tp_a(Li, Si, Ui) in lst2seq(decls_subst(z, tm_var(x), ds))) ==> (
      (forall u:nat, s':store :: (prefix_of(s.m, s'.m) && store_ok(k-1, s') &&
        V(k-1, u, Si, s')) ==> M(k-1, u, store_tp_lookup(u, s'), Li, s')) &&
      (forall u:nat, s':store :: (prefix_of(s.m, s'.m) && store_ok(k-1, s') && u in dom(s'.m) &&
