...
function store_lookup(n: nat, s: store): seq<def>
  requires n < |s.m|;
{
  s.m[n].snd
}
...
{
  /* msel */
  if (t.tm_msel? && t.o.tm_var? && value(t.a) && t.o.x < |s.m| &&
  ...
  /* sel */
  else if (t.tm_sel? && t.t.tm_var? && t.t.x < |s.m| &&
  ...
  then Some(P(tm_subst(t.y, tm_var(|s.m|), t.t'),
              Store(s.m+[P(t.Tc, lst2seq(defs_subst(t.y, tm_var(|s.m|), t.init)))])))
  else None
}
  ...
  decreases n;
{
  ...
  case tm_new(y, Tc, init, t') =>
    ...
    !tp_fn(y, T) &&
    typing(n-1, context_extend(ctx, y, Tc), s, t', T)
  case tm_sel(t, l) =>
...
{
  path(p1) && path(p2) && (
  (p1.tm_sel? && p1.t.tm_var? && p2.tm_var? && p1.t.x < |s.m| &&
